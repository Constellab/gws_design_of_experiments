...
class MilieuOptimizationProblem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        output_dict = dict(zip(self.all_outputs, y, strict=False))
        cv = (
            sum(max(0, self.thresholds[t] - output_dict[t]) for t in self.targets)
            if self.add_cv
            else 0
        )
...
class MyCallback(Callback):
    ...
    def notify(self, algorithm):
        ...
        if self.iterations and (gen % 10 == 0 or gen == self.iterations):
        ...
        if gen % self.save_every == 0:
            pd.DataFrame(self.history).to_csv(
                os.path.join(self.output_dir, "optimization_progress.csv"), index=False
            )
...
def optimize_model(name, base_model, param_dist, scorer, x, y, kf, pipelines, n_iter=200):
    ...
    pipe = Pipeline([("scaler", StandardScaler()), ("model", MultiOutputRegressor(search))])
...
def run_optimization(input_file: str, output_folder: str):
    ...
    scorer = make_scorer(
        lambda y_true, y_pred: r2_score(y_true, y_pred, multioutput="uniform_average")
    )
    pipelines = []

    print("[PROGRESS:10] Starting model optimization...")
    print("[PROGRESS:15] Optimizing RandomForest model...")
    pipelines = optimize_model(
        "RandomForest",
    ...
    pipelines = optimize_model(
        ...
        {"n_estimators": [100, 200, 300], "learning_rate": [0.01, 0.05, 0.2]},
        scorer,
        x,
        y,
        kf,
    ...
    pipelines = optimize_model(
        ...
        {"iterations": [100, 200, 300], "learning_rate": [0.01, 0.05, 0.2]},
        scorer,
        x,
        y,
        kf,
    ...
    importance_matrix = [
        est.best_estimator_.feature_importances_ for est in multi_output_model.estimators_
    ]
    ...
    constraints_df = pd.DataFrame(
        {"Feature": x.columns, "Lower_Bound_Used": low_mod, "Upper_Bound_Used": up_mod}
    )
    constraints_df.to_csv(
        os.path.join(output_folder, "constraints_used_in_optimization.csv"), index=False
    )
    ...
    if len(optimization_targets) == 1 and not add_cv_as_penalty:
        ...
        print(
            f"[INFO] Using GA algorithm with population={population_size}, iterations={iterations}"
        )
    else:
        algorithm = NSGA2(
        ...
        termination = get_termination("n_gen", iterations)
        print(
            f"[INFO] Using NSGA2 algorithm with population={population_size}, iterations={iterations}"
        )
    ...
    if res.F is not None and len(res.F) > 0:
        ...
        for target in optimization_targets:
            if target in full_thresholds:
                df_resultats[target + "_percent"] = (
                    df_resultats[target] / full_thresholds[target]
                ) * 100
        ...
        df_best = df_resultats.sort_values(
            by=["CV_percent"] + optimization_targets,
            ascending=[False] + [False] * len(optimization_targets),
        ).iloc[[0]]
