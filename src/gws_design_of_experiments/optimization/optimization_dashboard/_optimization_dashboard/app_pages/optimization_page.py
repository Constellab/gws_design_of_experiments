...
def render_first_page(path_output_dir: str):
    ...
    with best_solution:
        ...
        with col_tab:
            ...
            st.write(
                "Mean is calculated on the top 10 solutions as well as standard deviation. Optimal value is the value of the parameter in the best solution."
            )
        ...
        with col_plots:
            ...
            selected_params = st.selectbox(
                "Select parameter for boxplot:", options=available_params, index=0
            )
    ...
    with surface_explorer:
        ...
        z_pred = np.array(model.predict(xy_grid)).reshape(x_grid.shape)
    ...
    with observed_vs_predicted:
        ...
        if not os.path.exists(pred_file):
            st.warning(
                f"The {pred_file} file cannot be found. Please generate the predictions first."
            )
            return
    ...
    with feature_importance_matrix:
        ...
        if separate:
            ...
            fig = go.Figure()
            for target in selected_targets:
                fig.add_trace(go.Bar(name=target, x=imp_df.index, y=imp_df[target]))
            fig.update_layout(
                barmode="group",
                xaxis_title="Feature",
    ...
    with data_explorer:
        ...
        with cols[0]:
            ...
            sort_col = st.selectbox(
                "Sort by column:", df.columns.tolist(), index=df.columns.tolist().index(default_col)
            )
        with cols[1]:
        ...
        st.caption(f"Data are sorted by **{sort_col}** ({order} order).")
